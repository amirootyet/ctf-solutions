...
#
    ...
    #     print(c2.blocks[i])
        #shaObj = sha256()
        #shaObj.update(block)
        #print("\n\n\n SHA HASH HERE: ", shaObj.hexdigest())
        # i += 1
        # print("block index: " , i)

    ### saving the manipulated block
    # 
    #     
    c2.save_a_block(1010, 'jfrost.dat')
    #c2.blocks[1010].dump_doc(0)
    #c2.blocks[1010].dump_doc(1)
    

    ### reading the manipulated bytes


    # with open('jfrost.dat', 'rb') as fh:
    #     index = int(fh.read(16), 16)
    #     nonce = int(fh.read(16), 16)
    #     pid = int(fh.read(16), 16)
    #     rid = int(fh.read(16), 16)
    #     doc_count = int(fh.read(1), 10)
    #     score = int(fh.read(8), 16)
    #     sign = int(fh.read(1), 10)
    ...
    ### examining different possible data extensions

    # block = c2.blocks[1010]
    # for i in range(1, 15):
    #     filename = '{}.{}'.format(i, data_extension[i])
    #     with open(filename, 'wb') as fh:
    #         d = block.data[1]['data']
    #         fh.write(d)
    #     print('Document dumped as: %s' % (filename))
